  ...
  { // Results buffer scope, for implicit copyback
    ...
    if (implementation & ec_coarse) {
      ...
      if (isWeighted) {
        ...
        cl::sycl::event vertSum_event =
            sygraph::detail::Jaccard_RowSumKernel<true, int32_t, int32_t, WEIGHT_TYPE>::invoke(
                graph->number_of_vertices, graph->number_of_edges, graph->offsets, presumInd,
                &(graph->edge_data), vertWeights, q);
  #ifdef DEBUG_2
        q.wait();
